...
export const DiagnosisList: React.FC<Props> = props => {
	...
	return (
		<section className='bg-white rounded-lg border border-border p-4'>
			...
			{diagnoses.length === 0 && !adding && <p className='text-xs text-muted-foreground'>No diagnoses recorded.</p>}
			{diagnoses.map(d => (
				<div key={d.id} className='flex items-center justify-between py-2 border-t border-border first:border-0'>
					<div>
						...
						{d.is_primary && <span className='ml-2 text-xs text-primary font-medium'>Primary</span>}
					</div>
