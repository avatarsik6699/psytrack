...
export const SEChart: React.FC<Props> = props => {
	const seChartQuery = useSeChart(props.patientId);
	const rawPoints = seChartQuery.data ?? [];
	const points = rawPoints as SeSeverityDataPoint[];

	if (seChartQuery.isLoading) return <p className='text-xs text-muted-foreground'>Загрузка графика…</p>;
	...
	const chartData = allDates.map(d => {
		...
		for (const name of seNames) {
			const pt = points.find(p => p.date === d && p.se_name === name);
	...
	return (
		<div className='mt-4'>
			<h3 className='text-xs font-semibold text-muted-foreground mb-2'>Динамика побочных эффектов (тяжесть)</h3>
			<ResponsiveContainer width='100%' height={220}>
				<LineChart data={chartData} margin={{ top: 4, right: 8, left: 0, bottom: 4 }}>
					<CartesianGrid strokeDasharray='3 3' stroke='#e5e7eb' />
					<XAxis dataKey='date' tick={{ fontSize: 11 }} />
					<YAxis domain={[0, 4]} ticks={[0, 1, 2, 3, 4]} tick={{ fontSize: 11 }} />
					<Tooltip contentStyle={{ fontSize: 12 }} formatter={value => (value != null ? String(value) : '—')} />
