...
export function ScoreChart({ patientId }: Props) {
	const { data: series = [], isLoading } = useScoreChart(patientId);
	...
	// Build a unified timeline keyed by week-label
	const allDates = Array.from(
		new Set(series.flatMap(s => s.points.map(p => p.completed_at))),
	).sort();
	...
	// Delta chip: latest vs previous for each series
	const deltaChips = series.map(s => {
		const pts = s.points;
			...
			<div className='flex items-center justify-between mb-2'>
				...
				<div className='flex gap-2'>
					{deltaChips.map(chip => chip && (
						...
						>
							{chip.code} {chip.arrow}{Math.abs(chip.delta)}
						</span>
					))}
				</div>
			</div>
